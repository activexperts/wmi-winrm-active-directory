...
#################################################################################
# Script
#     DirectoryService-AccountLockedInGroup.ps1
# Description:
...
# Usage:
#     .\DirectoryService-AccountLockedInGroup.ps1 '<Domain>' '<Account>'
# Sample:
#     .\DirectoryService-AccountLockedInGroup.ps1 'DOMAIN01' 'Guest'
#################################################################################
...
{
  $res = 'UNCERTAIN: Invalid number of parameters - Usage: .\DirectoryService-AccountLockedInGroup.ps1 ''<Domain>'' ''<Account>'''
  echo $res
  exit 
}
...
{
  $res = 'ERROR: Account [' + $strDomain + '\' + $strAccount + '] is NOT locked'
  echo $res
  exit
}
...
{
  $res = 'SUCCESS: Account [' + $strDomain + '\' + $strAccount + '] is locked'
  echo $res
  exit
}
